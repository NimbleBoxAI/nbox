...
with console.status(f"[{T.st}]Polling while the model is being deployed ...[/{T.st}]", spinner=T.spinner) as status:
    ...
    if random.random() > 0.0:
        ...
        sleep(1)
        status.update(
            f"[[{T.clk}]{str(timedelta(seconds=int(time() - st)))[2:]}[/{T.clk}]] Current Status: [{T.inp}]deployment.in-progress"
        )
        sleep(3)
        if random.random() > 0.0:
            console.print(f"[[{T.clk}]{str(timedelta(seconds=int(time() - st)))[2:]}[/{T.clk}]] [{T.st}]Model is deployed at URL:")
            console.print(
                f"[[{T.clk}]{str(timedelta(seconds=int(time() - st)))[2:]}[/{T.clk}]]     https://depl.nbx.ai/yashbonde/its-britney-bish"
            )
        else:
            status.update(
                f"[[{T.clk}]{str(timedelta(seconds=int(time() - st)))[2:]}[/{T.clk}]] Current Status: [{T.fail}]deployment.failed"
            )
