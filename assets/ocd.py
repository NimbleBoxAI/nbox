...
class T:
  ...
  fail = "bold red"        # fail
  inp = "code bold yellow" # in-progress
...
with console.status("[{T.st}]Polling while the model is being deployed ...[/{T.st}]", spinner = "moon") as status:
  ...
  if random.random() > 0.0:
    ...
    if random.random() > 0.0:
      ...
      status.update(f"[[{T.clk}]{str(timedelta(seconds=int(time() - st)))[2:]}[/{T.clk}]] Current Status: [{T.fail}][code]deployment.failed")
      console.print(f"[[{T.clk}]{str(timedelta(seconds=int(time() - st)))[2:]}[/{T.clk}]] [{T.fail}]Model deployment failed")
  else:
    status.update(f"[[{T.clk}]{str(timedelta(seconds=int(time() - st)))[2:]}[/{T.clk}]] Current Status: [{T.fail}][code]conversion.failed")
