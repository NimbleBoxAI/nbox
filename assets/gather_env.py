...
def get_conda_packages(run_lambda, conda_packages_list=["torch", "torchvision", "numpy", "cudatoolkit", "magma", "mypy"]):
    if get_platform() == "win32":
        system_root = os.environ.get("SYSTEMROOT", "C:\\Windows")
        findstr_cmd = os.path.join(system_root, "System32", "findstr")
        _list = " ".join(conda_packages_list)
        grep_cmd = r'{} /R "{}"'.format(findstr_cmd, _list)
    else:
        _list = "\|".join(conda_packages_list)
        grep_cmd = r'grep "{}"'.format(_list)
